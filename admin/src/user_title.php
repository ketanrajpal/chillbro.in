...
{
	$sql="select ".$field." from `user_title` where `id`='".$_GET["update"]."'";
	$mysqli=connect();
...
{
	$mysqli=connect();
	$sql="DELETE FROM `user_title` where `id`='".$_GET["delete"]."'";
...
if($_SERVER["REQUEST_METHOD"]=="POST")
{
	...
	{
		$mysqli=connect();
		$sql="UPDATE `user_title` SET `name`='".$_POST["name"]."',`slug`='".$_POST["slug"]."',`by`='".$_POST["by"]."' where id='".$_GET["update"]."'";
		mysqli_query($mysqli,$sql);
		disconnect($mysqli);
	...
	{
		$mysqli=connect();
		$sql="INSERT into `user_title` (`name`,`slug`,`by`,`date`,`time`,`ip`)VALUES('".$_POST["name"]."','".$_POST["slug"]."','".$_POST["by"]."',now(),now(),'".$_SERVER["REMOTE_ADDR"]."')";
		mysqli_query($mysqli,$sql);
		disconnect($mysqli);
